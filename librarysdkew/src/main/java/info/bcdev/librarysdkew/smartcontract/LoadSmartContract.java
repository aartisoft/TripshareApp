...
public class LoadSmartContract {
    ...
    private static final String TAG = "LoadSmartContract";
    ...
    private class Token extends AsyncTask<Void, Void, Map<String, String>> {
        ...
        protected Map<String, String> doInBackground(Void... voids) {
            try {
                ...
                Log.d(TAG, "doInBackground:total send "+totalsupply);
                BigInteger tokenbalance = token.balanceOf(address).send();
                Log.d(TAG, "doInBackground:balance send  "+tokenbalance);
                String strtoken = tokenbalance.toString();
                String substrtoken = strtoken.substring(0,strtoken.length()-18);
                Log.d(TAG, "doInBackground:substr "+substrtoken);
                //Log.d(TAG, "doInBackground:balance integer "+Integer.valueOf(String.valueOf(tokenbalance)));
                ...
                result.put("tokenbalance",substrtoken);
                return result;
